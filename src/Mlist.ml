...
let rec mlist_of_list (l: 'a list) : 'a mlist =
    match l with
    | []     -> ret Nil  
    | hd::tl -> ret (Cons (hd, mlist_of_list tl))

let rec list_of_mlist (ml: 'a mlist) : 'a list mon =
    ml >>= function
    | Nil -> ret []
    | Cons (hd, tl) -> 
            list_of_mlist tl >>= fun tl ->
            ret (hd :: tl)
...
let split_mlist (ml: bool mlist) : (bool list * bool list) mon =
    ...
    mlist_eq (append ml1 ml2) ml >>= fun () ->
    list_of_mlist ml1 >>= fun l1 ->
    list_of_mlist ml2 >>= fun l2 ->
    ret (l1, l2)
